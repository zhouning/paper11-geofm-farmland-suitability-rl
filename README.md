...
- `paper/submission/`: IJAEOG submission-readiness audit, guarded submission text drafts, and the Phase 54-lineage-verified compressed-route formal manuscript package.
...
Run the read-only Phase 49 statistical robustness audit over the Phase 48 delta
table:
...
The current real Bishan audit reports `compressed_route_statistically_robust`:
pooled mean delta `0.4673011499`, `48 / 72` positive comparisons, one-sided
sign-test p `0.0031549137`, bootstrap CI95 `[0.2827829983, 0.6639974489]`,
minimum leave-one-tile mean `0.1613586660`, and minimum leave-one-seed mean
`0.3644002401`. This strengthens the compressed GeoFM base-reward claim while
leaving suitability reward, B2/B3, transfer, and independent agronomic
suitability claims blocked.

Run the read-only Phase 50 tile-seed cluster-level audit over the Phase 48
delta table:
...
The current Phase 50 status is `cluster_directional_support`: after aggregating
to the conservative tile-seed unit, the mean cluster delta remains
`0.4673011499` and `7 / 9` clusters are positive, but the one-sided cluster
sign-test p is `0.08984375`. This does not overturn Phase 48/49, but it narrows
wording: the compressed GeoFM route is supported on mean and row-level
robustness checks, while the sign-only cluster test is directional rather than
alpha-0.05 significant. Phase 51 resolves the magnitude question with an exact
...
Phase 54 verifies the formal artifact lineage for the Phase 52/53 evidence. The
current status is `artifact_lineage_consistent`: Phase 50 cluster means
recomputed from the authoritative Phase 48 delta table, Phase 51 signed-rank
statistics recomputed from the authoritative cluster CSV, and Phase 53
cluster-mean support recomputed from the same cluster CSV reproduce the formal
manuscript values.
...
Run the Phase 39 independent-label audit after the Phase 2 real feature table
exists:
...
`source_category`, `source_dlbm`, and `source_dlmc`. Expected local artifacts
are `phase39_label_inventory.csv`, `phase39_label_readiness.csv`,
`phase39_label_registry_template.csv`, `phase39_independent_label_audit.json`,
and `phase39_independent_label_audit.md`.
...
- Submission bundle preflight report: `paper/submission/final/Paper11_phase47_submission_preflight.json`
...
- Phase 54 artifact-lineage consistency audit: `paper/phase28_results/23_phase54_artifact_lineage_consistency.md`
...
- Phase 54 artifact-lineage consistency runner: `experiments/phase54_artifact_lineage_consistency/run_phase54_artifact_lineage_consistency.py`
...
- Phase 54 artifact-lineage consistency module: `src/paper11_geofm/phase54_artifact_lineage_consistency.py`
