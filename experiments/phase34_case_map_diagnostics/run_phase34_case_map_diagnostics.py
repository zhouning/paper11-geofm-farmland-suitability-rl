...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the read-only Paper11 Phase 34 case-map diagnostics over "
            "existing Phase 33 matched pilot outputs."
        )
    )
    parser.add_argument(
        "--phase33-output-dirs",
    ...
    try:
        ...
        paths = write_phase34_case_map_diagnostics_artifacts(
            analysis,
            args.output_dir,
        )
    ...
    print(
        "Phase 34 case-map status: "
        f"{analysis['phase34_case_map_status']}"
    )
    print(f"Case summary CSV: {paths['case_summary_csv']}")
    print(f"Case-map blocks CSV: {paths['case_map_blocks_csv']}")
